...
db_name = 'imdb'
db = db_name
host = 'database-2.cxcqxpvbnnwo.us-east-2.rds.amazonaws.com'
connection = connect(host=host, user="admin", password="newPassword", db=db)
cursor = connection.cursor()
db_setup = db_name + "_setup"
connection_setup = connect(host=host, user="admin", password="newPassword", db=db_setup)
cursor_setup = connection_setup.cursor()
db_bn = db_name + "_BN"
connection_bn = connect(host=host, user="admin", password="newPassword", db=db_bn)
    ...
    for j in values[i]:
        ...
        for k in range(len(rules[i])):
            if states[i][k] ==  0:
                ...
                matrix = 1 - matrices[functors[i][k]] if j[k + multiples[i]] == 'F' else matrices[functors[i][k]]
                unmasked_matrices.append(matrix)

            elif states[i][k] == 3:
                if j[k + multiples[i]] == 'N/A':
                    matrix = 1 - matrices[attributes[functors[i][k]]]
                    unmasked_matrices.append(matrix)


                else:
                    matrix = torch.zeros_like(matrices[attributes[functors[i][k]]], device='cuda')
                    for l in range(len(relations[attributes[functors[i][k]]][functors[i][k]])):
                        if relations[attributes[functors[i][k]]][functors[i][k]][l] == j[k + multiples[i]]:
                            index1 = indices[keys[attributes[functors[i][k]]][0]][relations[attributes[functors[i][k]]][keys[attributes[functors[i][k]]][0]][l]]
                            index2 = indices[keys[attributes[functors[i][k]]][1]][relations[attributes[functors[i][k]]][keys[attributes[functors[i][k]]][1]][l]]
        ...
        ground_truth.append(torch.sum(result))
...
def train_motif():
    model.train()
    optimizer.zero_grad()
    z, x_recon = model.encode(x, train_pos_edge_index)
    ...
    for i in range(1, 11):
        ...
        tensor_to_assign = ((x_recon[:, i-1]) > 0.5).int().cpu().numpy()
        entities['nodes_table'][feature_name] = tensor_to_assign


    matrices['edges_table'] = A_pred
    ...
    for i in range(len(rules)):
        ...
        for j in values[i]:
            ...
            for k in range(len(rules[i])):
                if states[i][k] ==  0:
                    matrix = torch.zeros((len(entities[nodes[i][k]].index), 1), device='cuda', dtype=torch.float16)
                    for l in range(len(entities[nodes[i][k]][functors[i][k]])):
                        value = entities[nodes[i][k]][functors[i][k]][l]
                        if isinstance(j[k + multiples[i]], str):
                            value = str(value) if isinstance(value, (int, float)) else value
                    ...
                    for l in masks[i][k]:
                        matrix = torch.zeros_like(matrices[l[0]], device='cuda', dtype=torch.float16)
                        for m in range(len(entities[nodes[i][k]][functors[i][k]])):
                            if entities[nodes[i][k]][functors[i][k]][m] == j[k + multiples[i]]:
                                index = indices[keys[nodes[i][k]]][entities[nodes[i][k]][keys[nodes[i][k]]][m]]
                                if variables[i][k] == l[1]:
                    ...
                    if j[k + multiples[i]] == 'N/A':
                        ...
                        matrix = torch.zeros_like(matrices[attributes[functors[i][k]]], device='cuda', dtype=torch.float16)
                        for l in range(len(relations[attributes[functors[i][k]]][functors[i][k]])):
                            if relations[attributes[functors[i][k]]][functors[i][k]][l] == j[k + multiples[i]]:
                                index1 = indices[keys[attributes[functors[i][k]]][0]][relations[attributes[functors[i][k]]][keys[attributes[functors[i][k]]][0]][l]]
                                index2 = indices[keys[attributes[functors[i][k]]][1]][relations[attributes[functors[i][k]]][keys[attributes[functors[i][k]]][1]][l]]
...
#             print(torch.sum(result))
            predicted.append(torch.sum(result).cpu().item())
            del unmasked_matrices, masked_matrices, sorted_matrices, stacked_matrices, matrix
...
plt.subplot(1, 2, 1)
plt.plot(range(1, epochs + 1), auc_scores, marker='o', color='b')
plt.title('AUC Score Over Epochs')
plt.xlabel('Epoch')
plt.ylabel('AUC Score')

plt.subplot(1, 2, 2)
plt.plot(range(1, epochs + 1), ap_scores, marker='o', color='r')
plt.title('AP Score Over Epochs')
plt.xlabel('Epoch')
plt.ylabel('AP Score')
